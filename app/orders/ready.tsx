...
export default function ReadyForPickupScreen() {
    ...
    // Single source of truth for rider-assignment state, so the banner,
    // "Next Action" card, and "Assigned Rider" card can never disagree.
    ...
    return (
        <SafeAreaView className="flex-1 bg-white">
            ...
            <ScrollView className="flex-1 bg-[#F8F9FE]" showsVerticalScrollIndicator={false}>
                        ...
                        <View className="flex-row items-center pr-6">
                            {STEPS.map((step, i) => (
                                <React.Fragment key={i}>
                                    ...
                                    {i < STEPS.length - 1 && (
                                        <View
                                            className={`w-10 mx-1 -mt-5 ${step.completed ? 'border-primary' : 'border-slate-200'
                                                }`}
                    ...
                    <View className="flex-row items-center flex-1 mr-2">
                        ...
                        <View className="ml-2.5 flex-1">
                            ...
                            <Text className="text-slate-500 text-[10px] mt-0.5">
                                {riderAssigned
                                    ? 'Rider is on the way to pick up this order.'
                ...
                {/* 4. Order Items Card (includes price breakdown) */}
                <View className="mx-4 mt-5 bg-white border border-slate-100 rounded-[28px] p-4 shadow-sm">
                    ...
                    {/* Items */}
                    {(expanded ? ITEMS : ITEMS.slice(0, 2)).map((item, idx, visible) => (
                        <View
                            key={idx}
                            className={`flex-row items-center py-3 ${idx < visible.length - 1 ? 'border-b border-slate-100/60' : ''
                                }`}
                        >
                            ...
                            <View className="items-end justify-between py-0.5 h-12 flex-shrink-0 min-w-[56px]">
                    ...
                    {ITEMS.length > 2 && (
                        ...
                        >
                            <Text className="text-slate-600 font-bold text-xs mr-1">
                                {expanded ? 'Show less' : 'View more items'}
                            </Text>
                            <ChevronDown
                                size={14}
                                color="#64748b"
                ...
                {/* 5. Next Action Card — only shown before a rider is assigned */}
                {!riderAssigned && (
                    <View className="mx-5 mt-4 bg-purple-50/40 border border-purple-100 p-4 rounded-[28px] shadow-sm space-y-3">
                        <View className="flex-row justify-between items-center">
                            ...
                            <View className="bg-purple-100 px-2.5 py-0.5 rounded-full">
                                <Text className="text-primary text-[10px] font-bold">Awaiting rider</Text>
                            </View>
                        </View>
                        <Text className="text-slate-500 text-[11px]">
                            Assign a rider to hand over this order.
                        </Text>

                        <Pressable
                            onPress={() => setRiderAssigned(true)}
                            className="h-12 bg-primary rounded-2xl flex-row items-center justify-center shadow-md shadow-primary/30 active:bg-primary/90 mt-1"
                        >
                            <ShoppingBag size={16} color="white" />
                            <Text className="text-white font-bold text-xs ml-1.5">Assign Rider</Text>
                        </Pressable>
                    </View>
                )}
                {/* 6. Assigned Rider Card — only shown once a rider is assigned */}
                {riderAssigned && (
                    <View className="mx-5 mt-4 bg-white border border-slate-100 p-4 rounded-[28px] shadow-sm space-y-3">
                        ...
                        <View className="flex-row items-center justify-between pt-1">
                            <View className="flex-row items-center flex-1 mr-2">
                                ...
                                <View className="flex-1">
                                    <View className="flex-row items-center">
                                        <Text className="font-bold text-slate-900 text-sm mr-1.5">
                                            {RIDER.name}
                                        </Text>
                                        <View className="flex-row items-center">
                                            <Star size={12} color="#f59e0b" fill="#f59e0b" />
                                            <Text className="text-slate-900 font-bold text-[10px] ml-0.5">
                                                {RIDER.rating}
                                            </Text>
                                        </View>
                                    </View>
                                    <Text className="text-slate-500 text-[11px] mt-0.5">{RIDER.phone}</Text>
                                    <Text className="text-slate-400 text-[10px]">{RIDER.plate}</Text>
                                </View>
                            </View>

                            <View className="items-end mr-2">
                                <Text className="text-slate-400 text-[8px] font-bold uppercase">Arriving in</Text>
                                <Text className="text-slate-900 font-extrabold text-base">{RIDER.eta}</Text>
                                <Text className="text-slate-400 text-[9px]">{RIDER.distance}</Text>
                            </View>
                        ...
                        <Pressable
                            ...
                            className="h-11 bg-primary rounded-2xl flex-row items-center justify-center shadow-md shadow-primary/30 active:bg-primary/90 mt-1"
                        >
